...
func loadPasswordKey() ([]byte, error) {
	...
	if !os.IsNotExist(err) {
		return nil, fmt.Errorf("failed to read password key file %s: %w", PasswordKeyFile, err)
	}

	dir := filepath.Dir(PasswordKeyFile)
	if err := os.MkdirAll(dir, 0700); err != nil {
		return nil, fmt.Errorf("failed to create password key directory %s: %w", dir, err)
	}
	key := make([]byte, PasswordKeyByteSize)
	if _, err := rand.Read(key); err != nil {
		return nil, fmt.Errorf("failed to generate password key: %w", err)
	}
	encoded := base64.StdEncoding.EncodeToString(key)
	if err := os.WriteFile(PasswordKeyFile, []byte(encoded), 0600); err != nil {
		return nil, fmt.Errorf("failed to write password key file %s: %w", PasswordKeyFile, err)
	}
	return key, nil
}
